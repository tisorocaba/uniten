...
class Local_model extends CI_Model {
    ...
    public function aprovados($agenda, $vagas) {
        ...
        $query = $this->db->query("SELECT classificacao, (select nome from aluno where id = aluno_id) as nome FROM local_curso_aluno WHERE local_curso_id = ".$agenda." order by classificacao asc  limit 0," . $vagas);
        $dado = array();
        if ($query->num_rows() > 0) {
            foreach ($query->result_array() as $row) {
                $obj = new stdClass();
    ...
    public function suplentes($agenda, $vagas) {
        $query = $this->db->query("SELECT classificacao, (select nome from aluno where id = aluno_id) as nome FROM local_curso_aluno WHERE local_curso_id = " . $agenda . "  order by classificacao asc limit " . $vagas . " , 500");
        $dado = array();
        if ($query->num_rows() > 0) {
            foreach ($query->result_array() as $row) {
                $obj = new stdClass();
