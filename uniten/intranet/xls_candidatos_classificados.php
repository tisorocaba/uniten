...
$rs = $obj->_getConnection()->executeSQL('SELECT   aluno_id as id,
                      ...
                      WHERE local_curso_id = ' . $agenda . '  HAVING  classificacao > 0 ORDER BY classificacao ASC, aluno ASC ');

$user = unserialize($_SESSION['USER']);
logDao::gravaLog($user->login, 'xls_candidatos', 'Exportou: lista de candidatos classificados', $_REQUEST, '', 'Agenda: ' .$_REQUEST['agenda']);
exportMysqlToCsv($rs);

function exportMysqlToCsv($result, $filename = 'candidatos-classificados.csv') {
