...
if ($_REQUEST['busca'] == '' && $_REQUEST['local'] == '') {
    ...
    if ($_REQUEST['status'] == 0) {
        $sql = "select LC.id as cod, C.nome as curso, L.local, periodo, DATE_FORMAT(data_inicio, '%d/%m/%Y') as inicio, 
...
(select fantasia from empresa where id = LC.empresa_curso_id) as empresa
        ...
        $sql = "select LC.id as cod, C.nome as curso, L.local, periodo, DATE_FORMAT(data_inicio, '%d/%m/%Y') as inicio, DATE_FORMAT(data_termino, '%d/%m/%Y') as termino,
            (select sum(carga_horaria) as total from disciplina D, curso_disciplina CD where CD.curso_id = LC.curso_id and CD.disciplina_id = D.id ) as cargaHoraria, 
                            ((select count(aluno_id) from local_curso_aluno where local_curso_id = LC.id and status = 0) +
(select count(aluno_id) from local_curso_aluno where local_curso_id = LC.id and status = 1)+
(select count(aluno_id) from local_curso_aluno where local_curso_id = LC.id and status = 2) + 
(select count(aluno_id) from local_curso_aluno where local_curso_id = LC.id and status = 3) + 
...
(select fantasia from empresa where id = LC.empresa_curso_id) as empresa
         ...
         and local_id = L.id
         and curso_id = C.id
         and C.nome like '%" . $_REQUEST['busca'] . "%'
         order by LC.id desc";
        } else {
            $sql = "select LC.id as cod, C.nome as curso, L.local, periodo, DATE_FORMAT(data_inicio, '%d/%m/%Y') as inicio, DATE_FORMAT(data_termino, '%d/%m/%Y') as termino,
                        ((select count(aluno_id) from local_curso_aluno where local_curso_id = LC.id and status = 0) +
...
(select fantasia from empresa where id = LC.empresa_curso_id) as empresa
         ...
         and LC.local_id =" . $_REQUEST['local'] . " order by LC.id desc";
            ...
            $sql = "select LC.id as cod, C.nome as curso, L.local, periodo, DATE_FORMAT(data_inicio, '%d/%m/%Y') as inicio, DATE_FORMAT(data_termino, '%d/%m/%Y') as termino,
         (select count(aluno_id) from local_curso_aluno where local_curso_id = L.id and status = 0) as inscritos,
            (select count(aluno_id) from local_curso_aluno where local_curso_id = L.id and status = 1) as matriculados       
         from local_curso LC, curso C, local L 
         where status = " . $_REQUEST['status'] . "
         and local_id = L.id
         and curso_id = C.id
...
logDao::gravaLog($user->login, 'xls_agendas', 'Exportou: lista de agendas', $_REQUEST, '', 'Status: ' . $_REQUEST['status']);


exportMysqlToCsv($rs);

function exportMysqlToCsv($result, $filename = 'agendas.csv') {
