...
$rs = $obj->_getConnection()->executeSQL('SELECT   aluno_id as id,
                               ...
                               (SELECT UPPER(rg) FROM aluno WHERE id = aluno_id ) as rg,
                               (SELECT UPPER(email) FROM aluno WHERE id = aluno_id ) as email,
                               (SELECT UPPER(endereco) FROM aluno WHERE id = aluno_id ) as endereco,
                               (SELECT UPPER(numero) FROM aluno WHERE id = aluno_id ) as numero,
                               DATE_FORMAT((SELECT data_nascimento FROM aluno WHERE id = aluno_id ),"%d/%m/%Y") as data_nascimento,
                               (SELECT UPPER(bairro) FROM aluno WHERE id = aluno_id ) as bairro,
                               (SELECT UPPER(cidade) FROM aluno WHERE id = aluno_id ) as cidade,
                               
                               (SELECT UPPER(sexo) FROM aluno WHERE id = aluno_id ) as sexo,
                               (SELECT UPPER(cor) FROM aluno WHERE id = aluno_id ) as cor,     
                               ...
                               (SELECT UPPER(condicao_especial_qual) FROM aluno WHERE id = aluno_id ) as condicao_especial_qual, 
                      ...
                      WHERE local_curso_id = ' . $agenda . ' and status = 0 HAVING  classificacao = 0 ORDER BY classificacao ASC, aluno ASC ');

$user = unserialize($_SESSION['USER']);
logDao::gravaLog($user->login, 'xls_candidatos_desclassificados', 'Exportou: lista de candidatos desclassificados', $_REQUEST, '', 'Agenda: ' .$_REQUEST['agenda']);
exportMysqlToCsv($rs);

function exportMysqlToCsv($result, $filename = 'candidatos-desclassificados.csv') {
