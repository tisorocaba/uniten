...
logDao::gravaLog($user->login, 'relatoriosClasses', 'Acessou: lista de relatorios em andamento',$_REQUEST);
?>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
<p><span class="titulo">Relatórios de Classes &raquo; Em Andamento </span><br>
    <br />
    </p>
...
<table width="100%" cellpadding="3" cellspacing="1" class="lista" >
          ...
          <tr>
              <td>Status:
                <select name="status"  onchange="self.location=this.value">
                  <option value="principal.php?acao=relatoriosClasses" selected="selected">Andamento</option>
                  <option value="principal.php?acao=relatoriosClassesFinalizados">Finalizados</option>
                  <option value="principal.php?acao=relatoriosClassesTodos">Todos</option>
              </select></td>
              <td><a href="xls_relatorios_agendas.php?status=1&amp;busca=<?php echo @$cpbusca  ?>&amp;local=<?php echo @$_GET['local'] ?>" target="_blank">Exportar</a><a href=".php"></a></td>
          </tr>
        </table></td>
    </tr>
    <tr class="listaClara">
    ...
    <tr class="listaClara">
       ...
       <!--  <td align="center">Ativo</td> -->
        <td width="83" align="center">Diários</td>
        <td width="88" align="center">Av. Final</td>
        <td width="98" align="center">Desistência</td>
...
?>
    ...
    <tr class="<?php echo $linha ?>">
        ...
        <td width="206"><?php echo $obj->local->local ?></td>
        <td width="145"><?php echo data_br($obj->dataInicio) ?></td>
        ...
        <td width="143" align="center">
          <a href="curso/pesquisaInfra.php?agenda=<?php echo $obj->id ?>" class="cssAlunos" id=""> Alunos</a> &nbsp; | &nbsp;
         
          <a href="curso/pesquisaProfessorCadastro.php?agenda=<?php echo $obj->id  ?>" class="cssAlunos" id=""> Monitor</a>
         </td>
        ...
        <td align="center">
              <a href="alunosAvaliacaoFinal.php?agenda=<?php echo $obj->id ?>" class="cssAlunos" id="<?php echo $obj->id ?>2">
    ...
    if ($paginas > 1) {
        ...
        for ($i = 0; $i < $paginas; $i++) {
            ...
            if (marcaPagina(@$_GET['offset'], $limit) == $i + 1) {
                printf('[&nbsp<a href="%s&offset=%d" >%s</a>]', 'principal.php?acao=relatoriosClasses&busca=' . @$_GET['busca']."&local=".@$_GET['local'], $start, $exibir);
            } else {
                printf('&nbsp<a href="%s&offset=%d" >%s</a>', 'principal.php?acao=relatoriosClasses&busca=' . @$_GET['busca']."&local=".@$_GET['local'], $start, $exibir);
            }
        }
