...
if($user->local == 1){
    ...
    $where = 'a.status > 0 and a.local = '.$user->local; 
}
...
$obj->limit($offset, @$limit)
        ->find();
logDao::gravaLog($user->login, 'relatoriosClassesTodos', 'Acessou: lista de todos os relatorios',$_REQUEST);
?>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
<p><span class="titulo">Relatórios de Classes &raquo; Todos </span><br>
    <br />
    </p>

<script src="scripts/relatoriosClassesTodos.js"></script>
<table width="100%" cellpadding="3" cellspacing="1" class="lista" >
          ...
          <tr>
              <td>Status:
                <select name="status"  onchange="self.location=this.value">
                  ...
                  <option value="principal.php?acao=relatoriosClassesTodos" selected="selected">Todos</option>
              </select></td>
              <td><a href="xls_relatorios_agendas.php?status=0&amp;busca=<?php echo @$cpbusca  ?>&amp;local=<?php echo @$_GET['local'] ?>" target="_blank">Exportar</a><a href=".php"></a></td>
            </tr>
        </table></td>
    </tr>
    <tr class="listaClara">
    ...
    <tr class="listaClara">
       ...
       <!--  <td align="center">Ativo</td> -->
        <td width="74" align="center">Diários </td>
        <td width="85" align="center">Av. Final</td>
        <td width="107" align="center">Desistência</td>
...
?>
    ...
    <tr class="<?php echo $linha ?>">
        ...
        <td width="143" align="center"><a href="curso/pesquisaInfra.php?agenda=<?php echo $obj->id ?>" class="cssAlunos" id="<?php echo $obj->id ?>"> Alunos</a> &nbsp; | &nbsp; <a href="curso/pesquisaProfessorCadastro.php?agenda=<?php echo $obj->id  ?>" class="cssAlunos" id="<?php echo $obj->id ?>"> Monitor</a></td>
    ...
    if ($paginas > 1) {
        ...
        for ($i = 0; $i < $paginas; $i++) {
            ...
            if (marcaPagina(@$_GET['offset'], $limit) == $i + 1) {
                printf('[&nbsp<a href="%s&offset=%d" >%s</a>]', 'principal.php?acao=relatoriosClassesTodos&busca=' . @$_GET['busca']."&local=".@$_GET['local'], $start, $exibir);
            } else {
                printf('&nbsp<a href="%s&offset=%d" >%s</a>', 'principal.php?acao=relatoriosClassesTodos&busca=' . @$_GET['busca']."&local=".@$_GET['local'], $start, $exibir);
            }
        }
