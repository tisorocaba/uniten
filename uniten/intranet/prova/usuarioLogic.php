    ...
    switch ($_REQUEST ['acao']) {
        case 'gravar' :
            ...
            try {
                ...
                logDao::gravaLog($user->id, 'usuarioLogic', $_REQUEST ['acao'], $_POST);
                msg($msg);
                gotox("principal.php?acao=usuarios");
            } catch (Exception $exc) {
                logDao::gravaLog($user->id, 'usuarioLogic', $_REQUEST ['acao'], $_POST, $exc);
        ...
        case 'remover' :
            if (!empty($_GET ['id']) && $obj->get($_GET ['id']) == 1) {
                try {
                    $obj->removeAll('menus');
                    $obj->delete();
                    logDao::gravaLog($user->id, 'usuarioLogic', $_REQUEST ['acao'], $_REQUEST);
                    msg('OK: Usuário removido com sucesso!');
                    gotox("principal.php?acao=usuarios");
                } catch (Exception $exc) {
                    logDao::gravaLog($user->id, 'professorLogic', $_REQUEST ['acao'], $_REQUEST, $exc);
        ...
        case 'alterarsenha' :
            if (!empty($_REQUEST ['id']) && $obj->get($_REQUEST ['id']) == 1) {
                try {
                    ...
                    logDao::gravaLog($user->id, 'usuarioLogic', $_REQUEST ['acao'], $_REQUEST, $exc);
