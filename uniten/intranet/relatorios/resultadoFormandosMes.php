...
$mes = Aluno::staticGet(12)->escape($_REQUEST['mes']);
$ano = Aluno::staticGet(12)->escape($_REQUEST['ano']);

logDao::gravaLog($user->login, 'resultadoAlunoGeral.php', 'Visualizou: Formando por vagas - MÊs', $_REQUEST, ' Mes: ' . $mes . ' a ' . $ano);
...
$sql = "SELECT  
    ...
    (select percentual_formandos(vagas,id)) as aproveitamento




FROM local_curso LC
WHERE  MONTH(data_termino) = '{$mes}' 
AND year(data_termino)   = '{$ano}'
AND status = 2
ORDER BY  data_inicio, local_id ASC ";
...
<table width="100%" border="0" cellspacing="0" cellpadding="15">
    <tr>
        <td valign="top" style="text-align:justify;">
            <p><strong class="titulo">Relatórios &raquo; Formandos por Mês</strong> </p>
        </td>
    </tr>
...
<table width="100%" cellpadding="3" cellspacing="1" class="lista">
    <tr class="listaClara">
        <td width="1139" colspan="2"><strong>Mês Pesquisado: </strong><?php echo mostraMes($mes) ?> de <?php echo $ano ?> </td>
    </tr>
    ...
    <tr class="">
        <td><table width="100%" border="0">
                <tr align="center">
                    ...
                    <td width="7%" bgcolor="#CCCCCC" style="font-size:10px"><strong>Iniciaram</strong></td>
                    <td width="6%" bgcolor="#CCCCCC" style="font-size:10px"><strong>Formandos</strong></td>
                    <td width="7%" bgcolor="#CCCCCC" style="font-size:10px"><strong>%</strong></td>
                </tr>
                <?php
                $cont = 0;
                $i = 1;
                ...
                while ($row = mysql_fetch_array($rs)) {
                    ...
                    $totalAproveit += $row['aproveitamento'];
                    ...
                    <tr class="<?php echo $linha ?>" align="center" >
                        ...
                        <td style="font-size:10px"><?php echo utf8_decode($row['mes']) ?></td>

                        <td style="font-size:10px"><?php echo $row['cargahoraria'] ?></td>
                        <td style="font-size:10px"><?php echo $row['vagas'] ?></td>
                        <td style="font-size:10px"><?php echo $row['iniciaram'] ?></td>
                        <td style="font-size:10px"><?php echo $row['aprovados'] ?></td>
                        <td style="font-size:10px"><?php echo $row['aproveitamento'] ?> %</td>
                    </tr>
                    <?php
                    $i++;
                }
                ...
                <tr class="" align="center">
                    ...
                    <td bgcolor="#FFFF99" style="font-size:10px"><?php echo $totalIniciaram ?></td>
                    <td bgcolor="#FFFF99" style="font-size:10px"><?php echo $totalFormandos ?></td>
                    <td bgcolor="#FFFF99" style="font-size:10px"><?php echo number_format($totalAproveit / $i, 2) ?> %</td>
                </tr>
